...
if (picture) {
  ...
  if (!fs.existsSync(path.resolve(__dirname, picture))) {
      ...
      outputPrompt((outputType) => {
        if (outputType == "Shader code") {
          fs.writeFile("output.txt", content, (err) => {
            if (err) throw err;
            console.log("Finished! Please see output.txt");
          });
        } else {
          ...
          fs.mkdir(
            ...
            (err) => {
              ...
              fs.writeFile(
                process.cwd() +
                  "/rp_placeholder/assets/minecraft/shaders/core/rendertype_solid.fsh",
                fsh,
                () => {
                  if (err) throw err;
                  fs.writeFile(
                    ...
                    () => {
                      if (err) throw err;
                      let output = fs.createWriteStream("EmissiveTextures.zip");
                      ...
                      archive.finalize();
                      fs.rmdirSync(process.cwd() + "/rp_placeholder", {
                        recursive: true,
                      });
                    }
                  );
